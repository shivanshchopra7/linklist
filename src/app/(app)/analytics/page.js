...
export default async function AnalyticsPage() {
  ...
  const normalizeUrl = (url) => {
    try {
      ...
      return `${parsed.hostname}${parsed.pathname}`.toLowerCase(); // Normalize and lowercase
    } catch {
      return url.trim().toLowerCase(); // Fallback for invalid URLs
    }
  };

  // Fetch grouped views
  const groupedViews = await Event.aggregate([
    { $match: { type: "view", uri: normalizeUrl(page.uri) } },
    {
      $group: {
        _id: {
          $dateToString: {
  ...
  const clicks = await Event.find({
    type: "click",
    uri: { $in: page.links.map((link) => normalizeUrl(link.url)) },
  });
            ...
            >
              ...
              <div className="text-center">
                <div className="border rounded-md p-2">
                  <div className="text-lg text-black">
                    {clicks.filter(
                      (c) => normalizeUrl(c.uri) === normalizeUrl(link.url)
                    ).length}
                  </div>
                  <div className="text-gray-400 text-xs uppercase font-bold">
                    Clicks all time
