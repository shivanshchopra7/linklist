...
export default async function UserPage({params}) {
    ...
    return(
       ...
       <Image src={user.image} className="rounded-full w-full h-full object-cover" width={256} height={256} alt={'avatar'} />
          ...
          <div className="max-w-xl grid md:grid-cols-2 gap-6  mx-auto px-8">
            {page.links.map(link => (
                <Link href={link.url}  className="bg-indigo-800 rounded-lg border p-2 block flex">
                    ...
                    {link.icon && <FontAwesomeIcon className="w-8 h-8" icon={link.icon} />}
                        {!link.icon && (
                    ...
                    <div>
                        <h3>{link.title}</h3>
                        <p className="text-white/70 h-8 overflow-ellipsis">{link.subtitle}</p>
