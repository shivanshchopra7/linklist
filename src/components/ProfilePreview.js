...
export default function ProfilePreview({ page, user, theme }) {
    ...
    <div className="bg-black text-white ">
      ...
      <div className="flex gap-2 justify-center -mb-4 pb-4">
        {Object.keys(page.buttons).map((buttonKey) => (
          <Link key={buttonKey} href={buttonLink(buttonKey, page.buttons[buttonKey])} className="rounded-full bg-white text-blue-950 p-2 flex items-center justify-center">
      ...
      <div className="max-w-md md:mt-8 grid gap-6 mx-auto px-8">
        {page.links.map((link) => (
          <Link key={link.url} href={link.url} target="_blank" className="items-center rounded-full border-white/30 border p-2 flex">
